...
typedef struct TaskStatusState{
...
void HariMain(void)
{
	...
	unsigned char* pWindowImage2;	/* Window2 Image */
	...
	/* Setting Task status State */
	tss_a.ldtr = 0;
	tss_a.iomap = 0x40000000;

	tss_b.ldtr = 0;
	tss_b.iomap = 0x40000000;
	tss_b.eip = (int) &Run_TaskB;
	tss_b.eflags = 0x00000202; 		/* IF = 1; */
	tss_b.eax = 0;
	tss_b.ecx = 0;
	tss_b.edx = 0;
	tss_b.ebx = 0;
	tss_b.esp = Alloc_MemoryBlock(memoryManager, 64 * 1024) + 64 * 1024-8;	/* Stack space for task B */
	tss_b.ebp = 0;
	tss_b.esi = 0;
	tss_b.edi = 0;
	tss_b.es = 1 * 8;
	...
	/* Allocating to GDT */
	set_segment_descriptor(gdt + 3, 103, (int) &tss_a, AR_TSS32);		/*taskA  103byte를 gdt 3번으로 등록*/
	set_segment_descriptor(gdt + 4, 103, (int) &tss_b, AR_TSS32);

	/* loading into Task Resister */
	load_tr(3 * 8);			/* Loading gdt 3th information into Task Register */
	
	/* Exporting SheetManager */
	*((int*) (tss_b.esp+4))= (int)pSheetManager;		/* Argument : pSheetManager */
