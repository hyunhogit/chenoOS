...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	buf_cons = (unsigned char *) memman_alloc_4k(memman, 256 * 420);
	sheet_setbuf(sht_cons, buf_cons, 256, 420, -1); /* 투명색없음 */
	make_window8(buf_cons, 256, 420, "console", 0);
	make_textbox8(sht_cons, 8, 28, 240, 384, COL8_000000);
							...
							for (j = shtctl->top - 1; j > 0; j--) {
								...
								if (0 <= x && x < sht->bxsize && 0 <= y && y < sht->bysize) {
									if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
										...
										if (3 <= x && x < sht->bxsize - 3 && 3 <= y && y < 21) {/*title bar*/
											mmx = mx;	/* 윈도우 이동 모드로 */
											mmy = my;
										}
