		...
		if( FifoStatus( &keyFifo) + FifoStatus( &mouseFifo ) == 0 ){
			...
			if( FifoStatus( &keyFifo) != 0 ){
				...
				if( DecodeMouse( &mouseData , i) ==1 ){
					/* 데이터가 3바이트 모였으므로 표시 */
					sprintf(s, "%02X %02X %02X", mouseData.data[0], mouseData.data[1], mouseData.data[2]);
					boxfill8(address_boot_info->address_vram,address_boot_info->screen_xsize, COL8_008484, 32, 16, 32 + 8 * 8 - 1, 31);
