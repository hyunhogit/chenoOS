		...
		if( FifoStatus( &keyFifo) + FifoStatus( &mouseFifo ) == 0 ){
			...
			if( FifoStatus( &keyFifo) != 0 ){
				...
				if( DecodeMouse( &mouseData , i) ==1 ){
					...
					boxfill8( address_boot_info->address_vram , address_boot_info->screen_xsize , COL8_008484, 0, 0, 79, 15); /* 좌표 지운다 */
					putString( address_boot_info->address_vram , address_boot_info->screen_xsize , 0, 0, COL8_FFFFFF, s); /* 좌표 쓴다 */
