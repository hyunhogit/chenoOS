...
void inthandler20(int* esp){
	...
	for( i=0 ; i < MAX_TIMER ; i++ ) {
		if( timerManager.timer[i].flag == TIMER_FLAGS_USING ){
			timerManager.timer[i].timeOut--;
			if( timerManager.timer[i].timeOut == 0 ){
				FifoPut(timerManager.timer[i].pFifo , timerManager.timer[i].data );
			}
		}
	}
	return;
