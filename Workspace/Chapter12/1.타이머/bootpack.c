...
void HariMain(void)
{
	...
	FifoInit( &timerFifo1 , 8 , timerBuf1 );
	FifoInit( &timerFifo2 , 8 , timerBuf2 );
	FifoInit( &timerFifo3 , 8 , timerBuf3 );

	Init_Timer( timer1, &timerFifo1 , 1);
	...
	for(;;){
		...
		io_cli();	/* 우선 인터럽트 막자 */
		if( FifoStatus( &keyFifo) + FifoStatus( &mouseFifo ) + FifoStatus( &timerFifo1 ) + FifoStatus ( &timerFifo2 ) + FifoStatus ( &timerFifo3 ) == 0 ){
			io_sti();
		}else{
			...
			}else if( FifoStatus( &timerFifo1 ) != 0 ){
				i= FifoGet(&timerFifo1);	/*먼저읽어들인다. 비워두기 위해 */
				io_sti();
			...
			}else if( FifoStatus( &timerFifo2 ) != 0 ){
				i= FifoGet(&timerFifo2);	/*먼저읽어들인다. 비워두기 위해 */
				io_sti();
			...
			}else if( FifoStatus( &timerFifo3 ) != 0 ){
				i= FifoGet(&timerFifo3);	/*먼저읽어들인다. 비워두기 위해 */
				io_sti();
				if(i!=0){ /* 깜빡임 처리 */
					Init_Timer( timer3 , &timerFifo3 , 0); /* 다음은 0으로 */
