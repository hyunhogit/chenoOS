...
void HariMain(void)
{
	...
	Make_Window(pWindowImage2,160,52,"Key Input",1);
	...
	UpDown_Sheet(pSheetManager, pSheetWindow2, 4);
	UpDown_Sheet( pSheetManager,  pSheetMouse, 5);
	...
	Run_Task(pTaskB[2]);	// You must run the task that is set last. I don't know why
	Run_Task(pTaskB[1]);	
	Run_Task(pTaskB[0]);
...
void Run_TaskB(SheetManager* pSheetManager , int j ){	/* Don't Share variables with hariMain */
	...
	Make_Window(pWindowImage,160,52,s,0);
	Set_pImageOfSheet(pSheetWindow,pWindowImage,160,52,-1);
	Move_Sheet(pSheetManager,pSheetWindow,450-(170*j),335);
