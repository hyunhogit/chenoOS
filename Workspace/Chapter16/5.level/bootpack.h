...
typedef struct fifo{
	...
	Task* pTask;		/* Task that makes it wake up */
}FIFO;
...
typedef struct SheetManager{
	...
	Sheet* pSHEETs[MAX_SHEET];	/* sheet를 height순서대로 정리해둔 배열*/
...
typedef struct PRIORITYLEVEL{
	...
	Task* pTasks[ NUM_TASK_IN_LEVEL ];	/* For ordering */
}PriorityLevel;
...
typedef struct TASKMANAGER{
	...
	PriorityLevel priorityLevel[ NUM_LEVEL ];
}TaskManager;	
