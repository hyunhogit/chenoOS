...
void UpDown_Sheet(SheetManager* pSheetManager, Sheet* pSheet, int height){
	...
	if (oldHeight > height) {	/* 이전보다 낮아진다 */
		if (height >= 0) {
			...
			Draw_Region(pSheetManager,pSheet->xPosition,pSheet->yPosition,pSheet->xPosition+pSheet->xSize,pSheet->yPosition+pSheet->ySize,height); /* 새로운 레이어의 정보에 따라 화면을 다시 그린다 */
		}
...
void Draw_Region(SheetManager*  pSheetManager, int xStart, int yStart, int xEnd, int yEnd,int height){
	int h;
	int x;
	int y;/*index*/
	int xVideo;/*비디오 좌표*/
...
void Refresh_Sheet(SheetManager* pSheetManager,Sheet* pSheet,int xStart, int yStart, int xEnd, int yEnd){
	if (pSheet->height >= 0) { /* 만약 표시중이라면, 새로운 레이어의 정보에 따라 화면을 다시 그린다 */
		Draw_Region(pSheetManager , pSheet->xPosition+xStart, pSheet->yPosition+yStart , pSheet->xPosition+xEnd , pSheet->yPosition+yEnd , pSheet->height);
	}
	return;
}
...
void Move_Sheet(SheetManager* pSheetManager, Sheet* pSheet, int xPosition, int yPosition){
	...
	if (pSheet->height >= 0) { /* 만약 표시중이라면 */
		Draw_Region(pSheetManager,oldX,oldY,oldX+pSheet->xSize,oldY+pSheet->ySize,0);
		Draw_Region(pSheetManager,xPosition,yPosition,xPosition+pSheet->xSize,yPosition+pSheet->ySize,pSheet->height);
	}
	return;
}
