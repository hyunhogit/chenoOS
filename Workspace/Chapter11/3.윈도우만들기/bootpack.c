...
void HariMain(void)
{
	...
	pWindowImage=(unsigned char*)Alloc_MemoryBlock(memoryManager, 160*68);	
	init_screen(pBackGroundImage,pBootInfo->xSize,pBootInfo->ySize);	
	init_mouse(pMouseImage,99);
	Make_Window(pWindowImage,160,68,"Window");
	...
	Set_pImageOfSheet(pSheetWindow,pWindowImage,160,68,-1);
	putString( pWindowImage , 160 , 24, 28,COL8_000000, "Welcome to"); 
	putString( pWindowImage , 160 , 24, 44,COL8_000000, "HO-OS!"); 
	...
	for(;;){
		...
		if( FifoStatus( &keyFifo) + FifoStatus( &mouseFifo ) == 0 ){
			...
			if( FifoStatus( &keyFifo) != 0 ){
				...
				Draw_AllSheetOfRegion(pSheetManager,0,0,16,16);
				...
				if( DecodeMouse( &mouseData , i) ==1 ){
					...
					Draw_AllSheetOfRegion(pSheetManager,0,32,120,48);
					/* 마우스 커서의 이동 */
					...
					Draw_AllSheetOfRegion(pSheetManager,0,16,80,32);
