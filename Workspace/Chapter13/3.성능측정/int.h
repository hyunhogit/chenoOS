...
void inthandler20(int* esp){
	...
	for( i=0 ; i < timerManager.usingTimer ; i++ ) {/*타임아웃된 타이머들을 처리*/				
		if( timerManager.pTimer[i]->timeOut > timerManager.count ){	/* 타임아웃 된 타이머들 */
			break;		
		}
		timerManager.pTimer[i]->flag = TIMER_FLAGS_ALLOC	;
		FifoPut(timerManager.pTimer[i]->pFifo , timerManager.pTimer[i]->data );
	}
	timerManager.usingTimer-=i;
	for( j = 0 ; j < timerManager.usingTimer ; j++ ){/*타임아웃되지 타이머들을 앞으로 옮김*/
		timerManager.pTimer[j]=timerManager.pTimer[i+j];
	}
	if( timerManager.usingTimer > 0 ){	/*가장 첫번째게 다음 울릴 타이머*/
