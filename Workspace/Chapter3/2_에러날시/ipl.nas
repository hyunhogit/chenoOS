...
;디스크를 읽는다
		...
		MOV		ax,0x0820
		MOV		es,ax
		MOV		ch,0			;interrupt 13번 호출시 읽어들일 위치.실린더 0
		MOV		dh,0			;interrupt 13번 호출시 읽어들일 위치.헤더0(위에서 읽으면0, 아래서 읽으면 1)
		MOV		cl,2			;interrupt 13번 호출시 읽어들일 위치.섹터2(부트로더가 1번에 올라가고 2번에 운영체제 내용 올릴려고)
		
		MOV		si,0			;실패횟수를 count하는 register
retry:	
		MOV		ah,0x02			;interrupt 13번 호출시 디스크 read(2:read,3:write,4:검사,5:찾기)
		MOV		al,1			;1섹터크기만큼 읽겠다
