...
next:
		...
		MOV		cl,1			;cl이 섹터번호라 생각하면 되,섹터한바퀴 돌았으므로 다시 초기화
		MOV		dh,1			;dh는 헤더번호라 생각하면 되,이제 헤더를 올려야겠지
		CMP		dh,2			;dh는 0,1있으므로 2와 비교해서..
		JB		readloop		;dh<2라면 readloop로 다시 돌아가
		MOV		dh,0			;헤더 한바퀴 돌았으므로 초기화
		ADD		ch,1			;실린더번호 ch로 카운트
		CMP		ch,CYLS			;ch와 CYLS(=10)비교
		JB		readloop		;실린더번호 0~9까지는 다시 readloop로
...
fin:
		JMP		0xc200			;read해두고 원하는 곳으로 jump
