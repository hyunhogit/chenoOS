...
;디스크를 읽는다
		...
		MOV		ax,0x0820
		MOV		es,ax
		MOV		ch,0			;interrupt 13번에 호출시 실린더 0
		MOV		dh,0			;interrupt 13번에 호출시 헤더0(위에서 읽으면0, 아래서 읽으면 1)
		MOV		cl,2			;interrupt 13번에 호출시 섹터2(부트로더가 1번에 올라가고 2번에 운영체제 내용 올릴려고)
		
		MOV		ah,0x02			;interrupt 13번에 호출시 디스크 read(2:read,3:write,4:검사,5:찾기)
		MOV 		bx,0			;interrupt 13번에 호출시 버퍼어드레스(검사/찾기에 사용하므로 0으로 초기화)
		MOV		dl,0x00			;interrupt 13번에 호출시 드라이브번호(0:a드라이브)
		INT		0x13			;interrupt 13번에 호출(디스크 BIOS 호출)
		JC		error			;carry flag 1이면 error로 점프(interrupt 13번 호출 후 에러가 있으면 carry flag가 1로 바뀜)

;error 없을 시 "Hello, World!출력"
		MOV		si,msg
		JMP		putloop
